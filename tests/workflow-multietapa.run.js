...
(async () => {
    ...
    try {
        ...
        const processosMenu = await driver.wait(until.elementLocated(By.xpath("//span[@title='Processos']")), 500);
        await driver.executeScript("arguments[0].scrollIntoView(true);", processosMenu);
        await driver.wait(until.elementIsVisible(processosMenu), 500);
        await processosMenu.click();
        log('✅ Clicou em "Processos"');
        await captureStepScreenshot(driver, 'clicou-processos');

        const iniciarSolic = await driver.wait(until.elementLocated(By.xpath("//span[@title='Iniciar Solicitações']")), 500);
        await driver.executeScript("arguments[0].scrollIntoView(true);", iniciarSolic);
        await driver.wait(until.elementIsVisible(iniciarSolic), 500);
        await iniciarSolic.click();
        log('✅ Clicou em "Iniciar Solicitações"');
        await captureStepScreenshot(driver, 'clicou-iniciar-solicitacoes');

        const cadastroNode = await driver.wait(until.elementLocated(By.xpath("//span[@class='fancytree-title' and text()='Cadastro']")), 500);
        await driver.wait(until.elementIsVisible(cadastroNode), 500);
        await cadastroNode.click();
        log('✅ Clicou em "Cadastro"');
        await captureStepScreenshot(driver, 'clicou-cadastro');

        const cadastroClientes = await driver.wait(until.elementLocated(By.xpath("//td[@processid='WKF_CADASTRO_DE_CLIENTES']")), 500);
        await driver.wait(until.elementIsVisible(cadastroClientes), 500);
        ...
        await driver.wait(until.ableToSwitchToFrame(By.id('workflowView-cardViewer')), 500);
        ...
        await driver.wait(until.elementLocated(By.name('A1_NOME')), 500);
        ...
        // ENVIAR O PROCESSO
        await driver.sleep(500); // espera para garantir que o botão esteja visível
        ...
        try {
            ...
            await driver.wait(until.elementIsVisible(botaoEnviar), 500);
